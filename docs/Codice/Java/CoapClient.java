...
public class CoapClient {
	...
	/**
	 * Indicate, it the client-specific executor service is detached, or shutdown
	 * with this client.
	 */
	...
	/**
	 * Constructs a new CoapClient with the specified scheme, host, port and path as
	 * URI.
	 *
	...
	/**
	 * Gets the maximum amount of time that synchronous method calls will block and
	 * wait.
	 * <p>
	...
	/**
	 * Sets the maximum amount of time that synchronous method calls will block and
	 * wait. Setting this property to 0 will result in methods waiting infinitely.
	 * <p>
	 * If this property is {@code null}, the value is from configuration property
	 * {@link org.eclipse.californium.core.network.config.NetworkConfig.Keys#EXCHANGE_LIFETIME}
	 * of the effective endpoint.
	 ...
	 * <em>EXCHANGE_LIFETIME</em> (the default) in order to account for potential
	 * retransmissions of request and response messages.
	 * <p>
	 * When running over DTLS and the client is behind a NAT firewall, requests may
	 * frequently fail (run into timeout) due to the fact that the client has been
	 * assigned a new IP address or port by the firewall and the peer can no longer
	 * associate this client's address with the original DTLS session. In such cases
	 * it might be worthwhile to set the value of this property to a smaller number
	 * so that the timeout is detected sooner and a new session can be negotiated.
	 * You may also consider to use the automatic session resumption to pass such a
	 * NAT firewall, see DtlsConnectorConfig.autoResumptionTimeoutMillis.
	 * 
	 ...
	 * 
	 * All handlers will be invoked by this executor. Note that the client executor
	 * uses a user thread (not a daemon thread) that needs to be stopped to exit the
	 * program by calling {@link #shutdown()}.
	 *
	 * @return the CoAP client
	 ...
	 * 
	 * All handlers will be invoked by this executor. The executor will shutdown on
	 * {@link #shutdown()}.
	 * 
	...
	/**
	 * Let the client use early negotiation for the blocksize (16, 32, 64, 128, 256,
	 * 512, or 1024). Other values will be matched to the closest logarithm dualis.
	 * 
	...
	/**
	 * Sends aGET request with the specified Accept option and invokes the handler
	...
	/**
	 * Sends a POST request with the specified payload, the specified content format
	 * and the specified Accept option and blocks until the response is available.
	 * 
	 * @param payload the payload
	 * @param format  the Content-Format
	 * @param accept  the Accept option
	...
	/**
	 * Sends a POST request with the specified payload, the specified content format
	 * and accept and invokes the specified handler when a response arrives.
	 * 
	 * @param handler the Response handler
	 * @param payload the payload
	 * @param format  the Content-Format
	...
	/**
	 * Sends a PUT request with payload and required Content-Format and blocks until
	...
	/**
	 * Sends a PUT request with with the specified ETags in the If-Match option and
	...
	/**
	 * Sends a PUT request with the If-None-Match option set and blocks until the
	 * response is available.
	 * 
	 * @param payload the payload string
	 * @param format  the Content-Format
	...
	/**
	 * Sends an observe request and waits until it has been established whereupon
	...
	/**
	 * Sends an observe request with the specified Accept option and waits until it
	 * has been established whereupon the specified handler is invoked when a
	...
	/**
	 * Shutdown the client-specific executor service, when not detached. Only needed
	 * if {@link #useExecutor()} or {@link #setExecutor(ExecutorService)} are used
	 * (i.e., a client-specific executor service was set).
	 */
	...
	/*
	 * Asynchronously sends the specified request and invokes the specified handler
	 * when a response arrives.
	 *
	...
	/**
	 * Sends the specified observe request and waits for the response whereupon the
	 * specified handler is invoked when a notification arrives.
	 *
	 * @param request the request
	 * 
	 * @param handler the Response handler
	...
	/**
	 * Sends the specified observe request and invokes the specified handler each
	...
	/**
	 * Sends the specified request over the endpoint of the client if one is defined
	 * or over the default endpoint otherwise.
	 *
	 * @param request the request
	 * @return the request
	 */
	...
	/**
	 * Returns the effective endpoint that the specified request is supposed to be
	 * sent over. If an endpoint has explicitly been set to this CoapClient, this
	 * endpoint will be used. If no endpoint has been set, the client will
	...
	/*
	 * Applies the CoapClient#type (managed by useNONs, useCONs) to the specified
	 * request.
	 *
	 *
	 * @param request the request
	...
	private class MessageObserverImpl extends MessageObserverAdapter {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.eclipse.californium.core.coap.MessageObserverAdapter#responded(
		...
		/**
		 * Invoked when a response arrives (even if the response code is not successful,
		 * the response still was successfully transmitted).
		 *
		...
		/**
		 * Invokes the handler's method with the specified response. This method must be
		 * invoked by the client's executor if it defines one.
		 * 
	 ...
	 * observed resource arrives. It wraps the response into a CoapResponse and lets
	 * the executor invoke the handler's method.
	 */
	private class ObserveMessageObserverImpl extends MessageObserverImpl {
		...
		/**
		 * Checks if the specified response truly is a new notification and if, invokes
		 * the handler's method or drops the notification otherwise. Ordering and
		 * delivery must be done synchronized here to deal with race conditions in the
		 * stack.
		 */
		@Override
		protected void deliver(CoapResponse response) {
	 ...
	 * available in separate strings. This is in particular useful to add multiple
	 * queries to the URI.
	 */
